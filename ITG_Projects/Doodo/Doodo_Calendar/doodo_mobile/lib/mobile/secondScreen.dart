...
import 'dart:async';
import 'dart:convert';
import '../Componentes/bottom_navigation_bar.dart';
import '../login.dart';
...
class _SecondScreenState extends State<SecondScreen> {
  ...
  void initState() {
    ...
    timer?.cancel();
    timer = Timer.periodic(Duration(seconds: 5), (Timer t) => VeriffyNott());
  }
  ...
  Future<void> _showNotification() async {
    ...
    await flutterLocalNotificationsPlugin.show(
      0,
      'Foi-lhe atribuida uma nova tarefa!',
...
SharedPreferences prefs = await SharedPreferences.getInstance();
    dynamic id = prefs.getString('id');
    dynamic response = await http.get(Uri.parse(
        'https://services.interagit.com/API/api_Calendar.php?query_param=19&id=$id'));
    if (response.statusCode == 200) {
      setState(() {
        tt = json.decode(response.body);
        //print(tar);
            int n;
      n = int.parse(tt.length.toString());
      if (n >= length) {
        getAllProduct("wot");
        _showNotification();
      }
