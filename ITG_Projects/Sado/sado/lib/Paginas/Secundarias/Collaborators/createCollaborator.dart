...
class CreateCollaborator extends StatefulWidget {
  final String idCompany; // Define the type of id

  CreateCollaborator({Key? key, required this.idCompany}) : super(key: key);

  @override
  _CreateCollaboratorState createState() => _CreateCollaboratorState();
}

class _CreateCollaboratorState extends State<CreateCollaborator> {
  ...
  void sendUserData() async {
    ...
    final response = await http.post(
      ...
      body: {
        ...
        'country': country,
        'pwd': pwd,
        'codcompany': widget.idCompany,
        'codaccess': codAccess,
      },
    );

    if (response.statusCode == 200) {
      _showDialog(context, "Collaborator Register", "Registration was successful.", 1);
    } else {
      ...
      child: _isLoading
          ...
          : AlertDialog(
              ...
              title: Column(
                ...
                children: [
                  const Text("Add Collaborator", style: TextStyle(color: Colors.blue)),
                ],
              ),
              content: Form(
                key: _formKey,
