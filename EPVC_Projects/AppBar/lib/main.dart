  ...
  Future<void> _checkConnectionAndProceed() async {
    ...
    try {
      ...
      if (connectivityResult.first != ConnectivityResult.none) {
        hasInternet = await InternetConnectionChecker.createInstance().hasConnection;
      }
    } catch (e) {
      hasInternet = false;
    }
    ...
    if (!hasInternet) {
      setState(() {
        _isChecking = false;
        _errorMessage = 'Sem conexão com a Internet. Verifique sua rede e tente novamente.';
      });
      return;
    }
    ...
    try {
      final response = await http.get(
        Uri.parse('https://appbar.epvc.pt/API/appBarAPI_GET.php?query_param=ping')
      ...
      if (response.statusCode >= 200 && response.statusCode < 400) {
        ...
        setState(() {
          _isChecking = false;
          _errorMessage = 'Não foi possível conectar ao servidor. Erro: ${response.statusCode}';
        });
      }
    } catch (e) {
      setState(() {
