...
class _ProductPageState extends State<ProdutoPage> {
  ...
  void updateProduct(String id, Product product) async {
    ...
    var response = await http.get(
      Uri.parse(
          'http://api.gfserver.pt/appBarAPI_GET.php?query_param=6&id=$id&nome=$nome&preco=$preco&available=$qtd&categoria=$categoria'),
...
void removeProduct(String id) async {
  var response = await http.get(
    Uri.parse('http://api.gfserver.pt/appBarAPI_GET.php?query_param=7&id=$id'),
    //body: jsonEncode(product.toJson()),
  );
  print(response);
  if (response.statusCode == 200) {
